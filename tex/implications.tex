...
This effectively halves the chances of annihilation, allowing the cross section to double. In order to compare the limits to the theoretical predictions, I extracted data points from Figure 6 in \cite{Profumo_2018} by using a plot digitizer and I then interpolated them, in order to plot them and to find the intersections with the theoretical values.
...
If Sommerfeld enhancement is relevant at the time of freeze-out, the situation is different, because the cross section must be corrected by multiplying by the Sommerfeld enhancement factor from Eq. \eqref{eq:result}. The parameter \(\alpha \) must then satisfy the following equation:
...
After reviewing dark matter evidence and defining the Sommerfeld enhancement factor in Eq. \eqref{eq:sommerfeld_def}, I derived its expression assuming a Coulomb-like long-range interaction between two dark matter particles and obtained Eq. \eqref{eq:result}. This result was then applied in the context of a non-self-conjugate secluded dark matter model with a freeze-out cross section of \(\langle \sigma v \rangle _{relic} = 4.4\times 10^{-26} \mathrm{cm^3 s^{-1} } \). Although the approximation of a massless mediator was later found to be excessive, the analysis laid out in this chapter leads to significant qualitative considerations.

First of all, considering Sommerfeld enhancement at the time of freeze-out results in a non-linear relation between the strength of the interaction and the dark matter mass as shown in Fig. \ref{fig:alpha}. This, coupled with strong Sommerfeld enhancement in dwarf spheroidal galaxies (see Fig. \ref{fig:enhancement_factor} for a representation of the enhancement factor as a function of the relative velocity and the mass of the interacting dark matter particles), leads to significantly different theoretical predictions of the annihilation cross section as a function of the mass. Comparing this prediction with annihilation cross section limits coming from the Fermi-LAT collaboration in the case of \(4\tau \) leptons decay (see Fig. \ref{fig:comparison}) constrains the dark matter mass to be \(M_{DM} \gtrsim 9.9 \mathrm{TeV} \). If one mistakenly ignored Sommerfeld enhancement, this limit would drop to \(M_{DM} \gtrsim 80 \mathrm{GeV} \), which is two orders of magnitude off from the aforementioned result. Ignoring Sommerfeld enhancement would thus lead to badly wrong conclusions. This has deep experimental implications, because it determines the way experiments have to look for dark matter candidates.